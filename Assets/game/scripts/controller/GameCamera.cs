...
public class GameCamera : MonoBehaviour {
	...
	void LateUpdate()
	{
		if (target) 
		{
			...
			float y = IncrementTowards(transform.position.y, target.position.y + 16, trackSpeed);
            //float z = IncrementTowards(transform.position.z, target.position.z - 14, trackSpeed);
